...
def check_temporal_consistency(u: BCPNNode, v: BCPNNode, edge_type: str) -> bool:
    """
    校验因果时间一致性（严格对齐论文3.3.1节）
    所有类型的因果关系都必须满足：源节点异常不晚于目标节点异常
    """
    if u.break_time is None or v.break_time is None:
        return True  # 无法判断时间顺序，默认通过
    
    delta_t = (v.break_time - u.break_time).total_seconds()
    
    # 所有因果关系的绝对约束：因不能晚于果
    if delta_t < 0:
        ColorPrint.print_fail(
            f"时间一致性失败（{edge_type}）：[{u.node_id}] ({u.break_time.strftime('%H:%M:%S')}) "
            f"晚于 [{v.node_id}] ({v.break_time.strftime('%H:%M:%S')})，因果倒置"
        )
        return False
    
    # 不同传播类型的延迟约束
    if edge_type == "trigger":
        # trigger边：传播延迟必须在合理范围内
        if delta_t < config.MIN_TRIGGER_DELAY or delta_t > config.MAX_TRIGGER_DELAY:
            ColorPrint.print_warn(
                f"trigger边传播延迟异常：[{u.node_id}→{v.node_id}] 耗时 {delta_t:.1f}秒 "
                f"(正常范围：{config.MIN_TRIGGER_DELAY}-{config.MAX_TRIGGER_DELAY}秒)"
            )
    
    elif edge_type == "amplify":
        # amplify边：允许更宽的传播延迟范围
        if delta_t > config.MAX_AMPLIFY_DELAY:
            ColorPrint.print_warn(
                f"amplify边传播延迟过长：[{u.node_id}→{v.node_id}] 耗时 {delta_t:.1f}秒 "
                f"(最大允许：{config.MAX_AMPLIFY_DELAY}秒)"
            )
...
def calculate_time_consistency(u: BCPNNode, v: BCPNNode, edge_type: str = "amplify") -> float:
    """
    计算时间一致性得分，严格对齐论文3.3.3节
    区分trigger和amplify的时间特性
    """
    ...
    if edge_type == "trigger":
        min_delay = config.MIN_TRIGGER_DELAY
        max_delay = config.MAX_TRIGGER_DELAY
        optimal_delay = config.OPTIMAL_TRIGGER_DELAY
    else:  # amplify
        min_delay = config.MIN_AMPLIFY_DELAY
        max_delay = config.MAX_AMPLIFY_DELAY
        optimal_delay = config.OPTIMAL_AMPLIFY_DELAY
    
    # 超出最大延迟，时间一致性得分降低
    if delta_t > max_delay:
        return 0.3
    
    # 高斯衰减函数：离最优值越远，得分越低，但下降平缓
    time_strength = np.exp(-((delta_t - optimal_delay) ** 2) / (2 * (max_delay / 3) ** 2))
